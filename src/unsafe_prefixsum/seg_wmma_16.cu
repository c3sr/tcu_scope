...
void tryCUDA_UNSAFE_WMMA_SEGMENTED_PREFIXSUM_16(benchmark::State &state) {
  ...
  try {
    ...
    for (auto _ : state) {
      ...
      compute_wmma_segmented_prefixsum_16<SEGMENTS_PER_WARP, WARPS_PER_BLOCK, BLOCK_DIM>
  ...
  if (errors > 0) {
    printf("CUDA_UNSAFE_WMMA_SEGMENTED_PREFIXSUM_16 does not agree with SEQUENTIAL! "
           "%d errors!\n",
           errors);
  } else {
...
template <int SEGMENTS_PER_WARP, int WARPS_PER_BLOCK>
void CUDA_UNSAFE_WMMA_SEGMENTED_PREFIXSUM_16(benchmark::State &state) {
  cudaDeviceReset();
  try {
    tryCUDA_UNSAFE_WMMA_SEGMENTED_PREFIXSUM_16<SEGMENTS_PER_WARP, WARPS_PER_BLOCK>(state);
...
#define RUN_CUDA_WMMA0(SEGMENTS_PER_WARP, WARPS_PER_BLOCK)                               \
  BENCHMARK_TEMPLATE(                                                                    \
      CUDA_UNSAFE_WMMA_SEGMENTED_PREFIXSUM_16, SEGMENTS_PER_WARP, WARPS_PER_BLOCK)       \
      ->SEG_16_ARGS()                                                                    \
      ->UseManualTime();
