...
void tryCUDA_UNSAFE_WMMA_FULL_PREFIXSUM_3KERS_BLOCK(benchmark::State &state) {
...
template <int SEGMENT_SIZE, int WARPS_PER_BLOCK>
void CUDA_UNSAFE_WMMA_FULL_PREFIXSUM_3KERS_BLOCK(benchmark::State &state) {
  cudaDeviceReset();
  try {
    tryCUDA_UNSAFE_WMMA_FULL_PREFIXSUM_3KERS_BLOCK<SEGMENT_SIZE, WARPS_PER_BLOCK>(state);
...
#define BENCHMARK_PRIFIXSUM0(SEGMENT_SIZE, WARPS_PER_BLOCK)                              \
  BENCHMARK_TEMPLATE(CUDA_UNSAFE_WMMA_FULL_PREFIXSUM_3KERS_BLOCK, SEGMENT_SIZE,          \
                     WARPS_PER_BLOCK)                                                    \
      ->ARGS()                                                                           \
      ->UseManualTime()
