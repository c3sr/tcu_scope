...
void tryCUDA_UNSAFE_WMMA_SEGMENTED_PREFIXSUM_16N_BLOCK(benchmark::State &state) {
  ...
  try {
    ...
    for (auto _ : state) {
      ...
      compute_wmma_segmented_prefixsum_16n_block<SEGMENT_SIZE, WARPS_PER_BLOCK, BLOCK_DIM>
  ...
  if (errors > 0) {
    printf("CUDA_UNSAFE_WMMA_SEGMENTED_PREFIXSUM_16N_BLOCK does not agree with SEQUENTIAL! "
           "%d errors!\n",
           errors);
  } else {
...
static void iCUDA_UNSAFE_WMMA_SEGMENTED_PREFIXSUM_16N_BLOCK(benchmark::State &state) {
  cudaDeviceReset();
  try {
    tryCUDA_UNSAFE_WMMA_SEGMENTED_PREFIXSUM_16N_BLOCK<SEGMENT_SIZE, WARPS_PER_BLOCK>(
        state);
...
static void CUDA_UNSAFE_WMMA_SEGMENTED_PREFIXSUM_16N_BLOCK(benchmark::State &state) {
  const int segment_size = state.range(1);
  switch (segment_size) {
#define Dispatch(N)                                                                      \
  case N:                                                                                \
    iCUDA_UNSAFE_WMMA_SEGMENTED_PREFIXSUM_16N_BLOCK<N, WARPS_PER_BLOCK>(state);          \
    break
...
template <int WARPS_PER_BLOCK>
static void CUDA_UNSAFE_WMMA_TUNE_SEGMENTED_PREFIXSUM_16N_BLOCK(benchmark::State &state) {
  CUDA_UNSAFE_WMMA_SEGMENTED_PREFIXSUM_16N_BLOCK<WARPS_PER_BLOCK>(state);
}

#define RUN_CUDA_UNSAFE_WMMA_TUNE(TUNE_ARGS)                                             \
  BENCHMARK_TEMPLATE(CUDA_UNSAFE_WMMA_TUNE_SEGMENTED_PREFIXSUM_16N_BLOCK, 1)             \
      ->Apply(TUNE_ARGS)                                                                 \
      ->UseManualTime();                                                                 \
  BENCHMARK_TEMPLATE(CUDA_UNSAFE_WMMA_TUNE_SEGMENTED_PREFIXSUM_16N_BLOCK, 2)             \
      ->Apply(TUNE_ARGS)                                                                 \
      ->UseManualTime();                                                                 \
  BENCHMARK_TEMPLATE(CUDA_UNSAFE_WMMA_TUNE_SEGMENTED_PREFIXSUM_16N_BLOCK, 4)             \
      ->Apply(TUNE_ARGS)                                                                 \
      ->UseManualTime();                                                                 \
  BENCHMARK_TEMPLATE(CUDA_UNSAFE_WMMA_TUNE_SEGMENTED_PREFIXSUM_16N_BLOCK, 8)             \
      ->Apply(TUNE_ARGS)                                                                 \
      ->UseManualTime();                                                                 \
  BENCHMARK_TEMPLATE(CUDA_UNSAFE_WMMA_TUNE_SEGMENTED_PREFIXSUM_16N_BLOCK, 16)            \
      ->Apply(TUNE_ARGS)                                                                 \
      ->UseManualTime();
...
#define RUN_CUDA_WMMA(Args)                                                              \
  BENCHMARK_TEMPLATE(CUDA_UNSAFE_WMMA_SEGMENTED_PREFIXSUM_16N_BLOCK, 1)                  \
      ->Args()                                                                           \
      ->UseManualTime();                                                                 \
  BENCHMARK_TEMPLATE(CUDA_UNSAFE_WMMA_SEGMENTED_PREFIXSUM_16N_BLOCK, 2)                  \
      ->Args()                                                                           \
      ->UseManualTime();                                                                 \
  BENCHMARK_TEMPLATE(CUDA_UNSAFE_WMMA_SEGMENTED_PREFIXSUM_16N_BLOCK, 4)                  \
      ->Args()                                                                           \
      ->UseManualTime();                                                                 \
  BENCHMARK_TEMPLATE(CUDA_UNSAFE_WMMA_SEGMENTED_PREFIXSUM_16N_BLOCK, 8)                  \
      ->Args()                                                                           \
      ->UseManualTime();                                                                 \
  BENCHMARK_TEMPLATE(CUDA_UNSAFE_WMMA_SEGMENTED_PREFIXSUM_16N_BLOCK, 16)                 \
      ->Args()                                                                           \
      ->UseManualTime();
