...
void tryCUDA_UNSAFE_WMMA_SEGMENTED_PREFIXSUM_16N_OPT(benchmark::State &state) {
  ...
  try {
    ...
    for (auto _ : state) {
      ...
      compute_wmma_segmented_prefixsum_16n_opt<SEGMENT_SIZE, WARPS_PER_BLOCK, BLOCK_DIM>
  ...
  if (errors > 0) {
    printf("CUDA_UNSAFE_WMMA_SEGMENTED_PREFIXSUM_16N_OPT does not agree with SEQUENTIAL! "
           "%d errors!\n",
           errors);
  } else {
...
static void iCUDA_UNSAFE_WMMA_SEGMENTED_PREFIXSUM_16N_OPT(benchmark::State &state) {
  cudaDeviceReset();
  try {
    tryCUDA_UNSAFE_WMMA_SEGMENTED_PREFIXSUM_16N_OPT<SEGMENT_SIZE, WARPS_PER_BLOCK>(state);
...
static void CUDA_UNSAFE_WMMA_SEGMENTED_PREFIXSUM_16N_OPT(benchmark::State &state) {
  const int segment_size = state.range(1);
  switch (segment_size) {
#define Dispatch(N)                                                                      \
  case N:                                                                                \
    iCUDA_UNSAFE_WMMA_SEGMENTED_PREFIXSUM_16N_OPT<N, WARPS_PER_BLOCK>(state);            \
    break
...
template <int WARPS_PER_BLOCK>
static void CUDA_UNSAFE_WMMA_TUNE_SEGMENTED_PREFIXSUM_16N_OPT(benchmark::State &state) {
  CUDA_UNSAFE_WMMA_SEGMENTED_PREFIXSUM_16N_OPT<WARPS_PER_BLOCK>(state);
}

#define RUN_CUDA_UNSAFE_WMMA_TUNE(TUNE_ARGS)                                             \
  BENCHMARK_TEMPLATE(CUDA_UNSAFE_WMMA_TUNE_SEGMENTED_PREFIXSUM_16N_OPT, 1)               \
      ->Apply(TUNE_ARGS)                                                                 \
      ->UseManualTime();                                                                 \
  BENCHMARK_TEMPLATE(CUDA_UNSAFE_WMMA_TUNE_SEGMENTED_PREFIXSUM_16N_OPT, 2)               \
      ->Apply(TUNE_ARGS)                                                                 \
      ->UseManualTime();                                                                 \
  BENCHMARK_TEMPLATE(CUDA_UNSAFE_WMMA_TUNE_SEGMENTED_PREFIXSUM_16N_OPT, 4)               \
      ->Apply(TUNE_ARGS)                                                                 \
      ->UseManualTime();                                                                 \
  BENCHMARK_TEMPLATE(CUDA_UNSAFE_WMMA_TUNE_SEGMENTED_PREFIXSUM_16N_OPT, 8)               \
      ->Apply(TUNE_ARGS)                                                                 \
      ->UseManualTime();                                                                 \
  BENCHMARK_TEMPLATE(CUDA_UNSAFE_WMMA_TUNE_SEGMENTED_PREFIXSUM_16N_OPT, 16)              \
      ->Apply(TUNE_ARGS)                                                                 \
      ->UseManualTime();
...
#define RUN_CUDA_WMMA(Args)                                                              \
  BENCHMARK_TEMPLATE(CUDA_UNSAFE_WMMA_SEGMENTED_PREFIXSUM_16N_OPT, 1)                    \
      ->Args()                                                                           \
      ->UseManualTime();                                                                 \
  BENCHMARK_TEMPLATE(CUDA_UNSAFE_WMMA_SEGMENTED_PREFIXSUM_16N_OPT, 2)                    \
      ->Args()                                                                           \
      ->UseManualTime();                                                                 \
  BENCHMARK_TEMPLATE(CUDA_UNSAFE_WMMA_SEGMENTED_PREFIXSUM_16N_OPT, 4)                    \
      ->Args()                                                                           \
      ->UseManualTime();                                                                 \
  BENCHMARK_TEMPLATE(CUDA_UNSAFE_WMMA_SEGMENTED_PREFIXSUM_16N_OPT, 8)                    \
      ->Args()                                                                           \
      ->UseManualTime();                                                                 \
  BENCHMARK_TEMPLATE(CUDA_UNSAFE_WMMA_SEGMENTED_PREFIXSUM_16N_OPT, 16)                   \
      ->Args()                                                                           \
      ->UseManualTime();
