...
static void ORIGINAL_CUB_FULL_PREFIXSUM(benchmark::State &state) {
  ...
  if (errors > 0) {
    printf("ORIGINAL_CUB_FULL_PREFIXSUM does not agree with SEQUENTIAL! %d errors!\n", errors);
  } else {
...
BENCHMARK(ORIGINAL_CUB_FULL_PREFIXSUM)->ARGS()->UseManualTime();
