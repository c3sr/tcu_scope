...
void tryCUDA_WMMA_SEGMENTED_PREFIXSUM_16N_OPT(benchmark::State &state) {
  ...
  if (errors > 0) {
    printf("CUDA_WMMA_SEGMENTED_PREFIXSUM_16N_OPT does not agree with SEQUENTIAL! "
           "%d errors!\n",
           errors);
  } else {
...
static void iCUDA_WMMA_SEGMENTED_PREFIXSUM_16N_OPT(benchmark::State &state) {
  cudaDeviceReset();
  try {
    tryCUDA_WMMA_SEGMENTED_PREFIXSUM_16N_OPT<SEGMENT_SIZE, WARPS_PER_BLOCK>(state);
...
static void CUDA_WMMA_SEGMENTED_PREFIXSUM_16N_OPT(benchmark::State &state) {
  const int segment_size = state.range(1);
  switch (segment_size) {
#define Dispatch(N)                                                                      \
  case N:                                                                                \
    iCUDA_WMMA_SEGMENTED_PREFIXSUM_16N_OPT<N, WARPS_PER_BLOCK>(state);                   \
    break
...
template <int WARPS_PER_BLOCK>
static void CUDA_WMMA_TUNE_SEGMENTED_PREFIXSUM_16N_OPT(benchmark::State &state) {
  CUDA_WMMA_SEGMENTED_PREFIXSUM_16N_OPT<WARPS_PER_BLOCK>(state);
}

#define RUN_CUDA_WMMA_TUNE(TUNE_ARGS)                                                    \
  BENCHMARK_TEMPLATE(CUDA_WMMA_TUNE_SEGMENTED_PREFIXSUM_16N_OPT, 1)                      \
      ->Apply(TUNE_ARGS)                                                                 \
      ->UseManualTime();                                                                 \
  BENCHMARK_TEMPLATE(CUDA_WMMA_TUNE_SEGMENTED_PREFIXSUM_16N_OPT, 2)                      \
      ->Apply(TUNE_ARGS)                                                                 \
      ->UseManualTime();                                                                 \
  BENCHMARK_TEMPLATE(CUDA_WMMA_TUNE_SEGMENTED_PREFIXSUM_16N_OPT, 4)                      \
      ->Apply(TUNE_ARGS)                                                                 \
      ->UseManualTime();                                                                 \
  BENCHMARK_TEMPLATE(CUDA_WMMA_TUNE_SEGMENTED_PREFIXSUM_16N_OPT, 8)                      \
      ->Apply(TUNE_ARGS)                                                                 \
      ->UseManualTime();                                                                 \
  BENCHMARK_TEMPLATE(CUDA_WMMA_TUNE_SEGMENTED_PREFIXSUM_16N_OPT, 16)                     \
      ->Apply(TUNE_ARGS)                                                                 \
      ->UseManualTime();
...
#define RUN_CUDA_WMMA(Args)                                                              \
  BENCHMARK_TEMPLATE(CUDA_WMMA_SEGMENTED_PREFIXSUM_16N_OPT, 1)->Args()->UseManualTime(); \
  BENCHMARK_TEMPLATE(CUDA_WMMA_SEGMENTED_PREFIXSUM_16N_OPT, 2)->Args()->UseManualTime(); \
  BENCHMARK_TEMPLATE(CUDA_WMMA_SEGMENTED_PREFIXSUM_16N_OPT, 4)->Args()->UseManualTime(); \
  BENCHMARK_TEMPLATE(CUDA_WMMA_SEGMENTED_PREFIXSUM_16N_OPT, 8)->Args()->UseManualTime(); \
  BENCHMARK_TEMPLATE(CUDA_WMMA_SEGMENTED_PREFIXSUM_16N_OPT, 16)->Args()->UseManualTime();
