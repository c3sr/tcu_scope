...
__global__ void compute_cub_warp_segmented_prefixsum(half *d_in, half *d_out) {
  constexpr int num_warps = THREADS_PER_BLOCK / LOGICAL_THREADS_PER_WARP;
  const int offset        = blockIdx.x * blockDim.x + threadIdx.x;
  const int warp_id       = offset / WARP_SIZE;

  typedef WarpScan<half, LOGICAL_THREADS_PER_WARP> WarpScanT;
  __shared__ typename WarpScanT::TempStorage temp_storage[num_warps];

  WarpScanT(temp_storage[warp_id]).InclusiveSum(d_in[offset], d_out[offset]);
}

template <int THREADS_PER_BLOCK, int LOGICAL_THREADS_PER_WARP>
static void CUB_WARP_SEGMENTED_PREFIXSUM(benchmark::State &state) {
  ...
  try {
    ...
    for (auto _ : state) {
      ...
      compute_cub_warp_segmented_prefixsum<THREADS_PER_BLOCK, LOGICAL_THREADS_PER_WARP>
  ...
  if (errors > 0) {
    printf("CUB_SEGMENTED_PREFIXSUM_16 does not agree with SEQUENTIAL! %d "
           "errors!\n",
           errors);
  } else {
...
BENCHMARK_TEMPLATE(CUB_WARP_SEGMENTED_PREFIXSUM, 32, 16)->SEG_16_ARGS()->UseManualTime();
BENCHMARK_TEMPLATE(CUB_WARP_SEGMENTED_PREFIXSUM, 64, 16)->SEG_16_ARGS()->UseManualTime();
BENCHMARK_TEMPLATE(CUB_WARP_SEGMENTED_PREFIXSUM, 128, 16)->SEG_16_ARGS()->UseManualTime();
BENCHMARK_TEMPLATE(CUB_WARP_SEGMENTED_PREFIXSUM, 256, 16)->SEG_16_ARGS()->UseManualTime();
BENCHMARK_TEMPLATE(CUB_WARP_SEGMENTED_PREFIXSUM, 512, 16)->SEG_16_ARGS()->UseManualTime();
BENCHMARK_TEMPLATE(CUB_WARP_SEGMENTED_PREFIXSUM, 1024, 16)
    ->SEG_16_ARGS()
    ->UseManualTime();

BENCHMARK_TEMPLATE(CUB_WARP_SEGMENTED_PREFIXSUM, 32, 32)->SEG_32_ARGS()->UseManualTime();
BENCHMARK_TEMPLATE(CUB_WARP_SEGMENTED_PREFIXSUM, 64, 32)->SEG_32_ARGS()->UseManualTime();
BENCHMARK_TEMPLATE(CUB_WARP_SEGMENTED_PREFIXSUM, 128, 32)->SEG_32_ARGS()->UseManualTime();
BENCHMARK_TEMPLATE(CUB_WARP_SEGMENTED_PREFIXSUM, 256, 32)->SEG_32_ARGS()->UseManualTime();
BENCHMARK_TEMPLATE(CUB_WARP_SEGMENTED_PREFIXSUM, 512, 32)->SEG_32_ARGS()->UseManualTime();
BENCHMARK_TEMPLATE(CUB_WARP_SEGMENTED_PREFIXSUM, 1024, 32)
    ->SEG_32_ARGS()
    ->UseManualTime();
