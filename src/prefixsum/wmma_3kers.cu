...
void tryCUDA_WMMA_FULL_PREFIXSUM_3KERS(benchmark::State &state) {
...
template <int SEGMENT_SIZE, int WARPS_PER_BLOCK>
void CUDA_WMMA_FULL_PREFIXSUM_3KERS(benchmark::State &state) {
  cudaDeviceReset();
  try {
    tryCUDA_WMMA_FULL_PREFIXSUM_3KERS<SEGMENT_SIZE, WARPS_PER_BLOCK>(state);
...
#define BENCHMARK_PRIFIXSUM0(SEGMENT_SIZE, WARPS_PER_BLOCK)                              \
  BENCHMARK_TEMPLATE(CUDA_WMMA_FULL_PREFIXSUM_3KERS, SEGMENT_SIZE, WARPS_PER_BLOCK)      \
      ->ARGS()                                                                           \
      ->UseManualTime()
