...
void tryCUDA_WMMA_SEGMENTED_PREFIXSUM_256N(benchmark::State &state) {
  ...
  if (errors > 0) {
    printf("CUDA_WMMA_SEGMENTED_PREFIXSUM_256N does not agree with SEQUENTIAL! %d "
           "errors!\n",
           errors);
  } else {
...
template <size_t SEGMENT_SIZE, int WARPS_PER_BLOCK>
static void iCUDA_WMMA_SEGMENTED_PREFIXSUM_256N(benchmark::State &state) {
  cudaDeviceReset();
  try {
    tryCUDA_WMMA_SEGMENTED_PREFIXSUM_256N<SEGMENT_SIZE, WARPS_PER_BLOCK>(state);
...
static void CUDA_WMMA_SEGMENTED_PREFIXSUM_256N(benchmark::State &state) {
  const int segment_size = state.range(1);
  switch (segment_size) {
#define Dispatch(N)                                                                      \
  case N:                                                                                \
    iCUDA_WMMA_SEGMENTED_PREFIXSUM_256N<N, WARPS_PER_BLOCK>(state);                      \
    break
...
template <int WARPS_PER_BLOCK>
static void CUDA_WMMA_TUNE_SEGMENTED_PREFIXSUM_256N(benchmark::State &state) {
  CUDA_WMMA_SEGMENTED_PREFIXSUM_256N<WARPS_PER_BLOCK>(state);
}
#define RUN_CUDA_WMMA_TUNE(TUNE_ARGS)                                                    \
  BENCHMARK_TEMPLATE(CUDA_WMMA_TUNE_SEGMENTED_PREFIXSUM_256N, 1)                         \
      ->Apply(TUNE_ARGS)                                                                 \
      ->UseManualTime();                                                                 \
  BENCHMARK_TEMPLATE(CUDA_WMMA_TUNE_SEGMENTED_PREFIXSUM_256N, 2)                         \
      ->Apply(TUNE_ARGS)                                                                 \
      ->UseManualTime();                                                                 \
  BENCHMARK_TEMPLATE(CUDA_WMMA_TUNE_SEGMENTED_PREFIXSUM_256N, 4)                         \
      ->Apply(TUNE_ARGS)                                                                 \
      ->UseManualTime();                                                                 \
  BENCHMARK_TEMPLATE(CUDA_WMMA_TUNE_SEGMENTED_PREFIXSUM_256N, 8)                         \
      ->Apply(TUNE_ARGS)                                                                 \
      ->UseManualTime();                                                                 \
  BENCHMARK_TEMPLATE(CUDA_WMMA_TUNE_SEGMENTED_PREFIXSUM_256N, 16)                        \
      ->Apply(TUNE_ARGS)                                                                 \
      ->UseManualTime();
...
#define RUN_CUDA_WMMA(Args)                                                              \
  BENCHMARK_TEMPLATE(CUDA_WMMA_SEGMENTED_PREFIXSUM_256N, 1)->Args()->UseManualTime();    \
  BENCHMARK_TEMPLATE(CUDA_WMMA_SEGMENTED_PREFIXSUM_256N, 2)->Args()->UseManualTime();    \
  BENCHMARK_TEMPLATE(CUDA_WMMA_SEGMENTED_PREFIXSUM_256N, 4)->Args()->UseManualTime();    \
  BENCHMARK_TEMPLATE(CUDA_WMMA_SEGMENTED_PREFIXSUM_256N, 8)->Args()->UseManualTime();    \
  BENCHMARK_TEMPLATE(CUDA_WMMA_SEGMENTED_PREFIXSUM_256N, 16)->Args()->UseManualTime();
