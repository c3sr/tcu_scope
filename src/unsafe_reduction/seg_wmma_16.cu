...
void tryCUDA_UNSAFE_WMMA_SEGMENTED_REDUCTION_16(benchmark::State &state) {
  const size_t num_segments = state.range(0);
  const size_t segment_size = state.range(1);

  if (segment_size != 16) {
    state.SkipWithError("segment size must be 16");
  }

  const int BLOCK_DIM          = WARPS_PER_BLOCK * WARP_SIZE;
  const size_t num_elements    = num_segments * segment_size;
  const int segments_per_block = WARPS_PER_BLOCK * SEGMENTS_PER_WARP * 16;
  ...
  try {
    ...
    for (auto _ : state) {
      ...
      compute_wmma_segmented_reduction_16<SEGMENTS_PER_WARP, WARPS_PER_BLOCK, BLOCK_DIM>
  ...
  if (errors > 0) {
    printf(
        "CUDA_UNSAFE_WMMA_SEGMENTED_REDUCTION_16 does not agree with SEQUENTIAL! %d errors!\n",
        errors);
  } else {
...
template <int SEGMENTS_PER_WARP, int WARPS_PER_BLOCK>
void CUDA_UNSAFE_WMMA_SEGMENTED_REDUCTION_16(benchmark::State &state) {
  cudaDeviceReset();
  try {
    tryCUDA_UNSAFE_WMMA_SEGMENTED_REDUCTION_16<SEGMENTS_PER_WARP, WARPS_PER_BLOCK>(state);
...
#define RUN_CUDA_WMMA0(SEGMENTS_PER_WARP, WARPS_PER_BLOCK)                               \
  BENCHMARK_TEMPLATE(                                                                    \
      CUDA_UNSAFE_WMMA_SEGMENTED_REDUCTION_16, SEGMENTS_PER_WARP, WARPS_PER_BLOCK)       \
      ->SEG_16_ARGS()                                                                    \
      ->UseManualTime();
