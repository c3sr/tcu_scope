...
void tryCUDA_UNSAFE_WMMA_FULL_REDUCTION_2KERS_BLOCK(benchmark::State &state) {
...
template <size_t BASE_SEGMENT_SIZE, size_t SEGMENT_SIZE, int WARPS_PER_BLOCK>
void CUDA_UNSAFE_WMMA_FULL_REDUCTION_2KERS_BLOCK(benchmark::State &state) {
  cudaDeviceReset();
  try {
    tryCUDA_UNSAFE_WMMA_FULL_REDUCTION_2KERS_BLOCK<BASE_SEGMENT_SIZE, SEGMENT_SIZE,
...
void CUDA_UNSAFE_WMMA_FULL_REDUCTION_2KERS_BLOCK_BASE_16(benchmark::State &state) {
  CUDA_UNSAFE_WMMA_FULL_REDUCTION_2KERS_BLOCK<16, SEGMENT_SIZE, WARPS_PER_BLOCK>(state);
}

template <size_t SEGMENT_SIZE, int WARPS_PER_BLOCK>
void CUDA_UNSAFE_WMMA_FULL_REDUCTION_2KERS_BLOCK_BASE_256(benchmark::State &state) {
  CUDA_UNSAFE_WMMA_FULL_REDUCTION_2KERS_BLOCK<256, SEGMENT_SIZE, WARPS_PER_BLOCK>(state);
}

#define BENCHMARK_REDUCTION0(SEGMENT_SIZE, WARPS_PER_BLOCK)                              \
  BENCHMARK_TEMPLATE(CUDA_UNSAFE_WMMA_FULL_REDUCTION_2KERS_BLOCK_BASE_16, SEGMENT_SIZE,  \
                     WARPS_PER_BLOCK)                                                    \
      ->ARGS()                                                                           \
      ->UseManualTime();                                                                 \
  BENCHMARK_TEMPLATE(CUDA_UNSAFE_WMMA_FULL_REDUCTION_2KERS_BLOCK_BASE_256, SEGMENT_SIZE, \
                     WARPS_PER_BLOCK)                                                    \
      ->ARGS()                                                                           \
      ->UseManualTime()
