...
void tryCUDA_WMMA_SEGMENTED_REDUCTION_16(benchmark::State &state) {
  ...
  if (errors > 0) {
    printf(
        "CUDA_WMMA_SEGMENTED_REDUCTION_16 does not agree with SEQUENTIAL! %d errors!\n",
        errors);
  } else {
...
template <int SEGMENTS_PER_WARP, int WARPS_PER_BLOCK>
void CUDA_WMMA_SEGMENTED_REDUCTION_16(benchmark::State &state) {
  cudaDeviceReset();
  try {
    tryCUDA_WMMA_SEGMENTED_REDUCTION_16<SEGMENTS_PER_WARP, WARPS_PER_BLOCK>(state);
...
#define RUN_CUDA_WMMA0(SEGMENTS_PER_WARP, WARPS_PER_BLOCK)                               \
  BENCHMARK_TEMPLATE(                                                                    \
      CUDA_WMMA_SEGMENTED_REDUCTION_16, SEGMENTS_PER_WARP, WARPS_PER_BLOCK)              \
      ->SEG_16_ARGS()                                                                    \
      ->UseManualTime();
