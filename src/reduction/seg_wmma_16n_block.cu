...
static void tryCUDA_WMMA_SEGMENTED_REDUCTION_16N_BLOCK(benchmark::State &state) {
  ...
  if (errors > 0) {
    printf(
        "CUDA_WMMA_SEGMENTED_REDUCTION_16N_BLOCK does not agree with SEQUENTIAL! %d errors!\n",
        errors);
  } else {
...
template <size_t SEGMENT_SIZE, int WARPS_PER_BLOCK>
static void iCUDA_WMMA_SEGMENTED_REDUCTION_16N_BLOCK(benchmark::State &state) {
  cudaDeviceReset();
  try {
    tryCUDA_WMMA_SEGMENTED_REDUCTION_16N_BLOCK<SEGMENT_SIZE, WARPS_PER_BLOCK>(state);
...
static void CUDA_WMMA_SEGMENTED_REDUCTION_16N_BLOCK(benchmark::State &state) {
  const int segment_size = state.range(1);
  switch (segment_size) {
#define Dispatch(N)                                                                      \
  case N:                                                                                \
    iCUDA_WMMA_SEGMENTED_REDUCTION_16N_BLOCK<N, WARPS_PER_BLOCK>(state);                 \
    break
...
template <int WARPS_PER_BLOCK>
static void CUDA_WMMA_TUNE_SEGMENTED_REDUCTION_16N_BLOCK(benchmark::State &state) {
  CUDA_WMMA_SEGMENTED_REDUCTION_16N_BLOCK<WARPS_PER_BLOCK>(state);
}

#define RUN_CUDA_WMMA_TUNE(TUNE_ARGS)                                                    \
  BENCHMARK_TEMPLATE(CUDA_WMMA_TUNE_SEGMENTED_REDUCTION_16N_BLOCK, 1)                    \
      ->Apply(TUNE_ARGS)                                                                 \
      ->UseManualTime();                                                                 \
  BENCHMARK_TEMPLATE(CUDA_WMMA_TUNE_SEGMENTED_REDUCTION_16N_BLOCK, 2)                    \
      ->Apply(TUNE_ARGS)                                                                 \
      ->UseManualTime();                                                                 \
  BENCHMARK_TEMPLATE(CUDA_WMMA_TUNE_SEGMENTED_REDUCTION_16N_BLOCK, 4)                    \
      ->Apply(TUNE_ARGS)                                                                 \
      ->UseManualTime();                                                                 \
  BENCHMARK_TEMPLATE(CUDA_WMMA_TUNE_SEGMENTED_REDUCTION_16N_BLOCK, 8)                    \
      ->Apply(TUNE_ARGS)                                                                 \
      ->UseManualTime();                                                                 \
  BENCHMARK_TEMPLATE(CUDA_WMMA_TUNE_SEGMENTED_REDUCTION_16N_BLOCK, 16)                   \
      ->Apply(TUNE_ARGS)                                                                 \
      ->UseManualTime();
...
// RUN_CUDA_WMMA_TUNE(Tuning17_x_26);
RUN_CUDA_WMMA_TUNE(Tuning16_x_30);

#define RUN_CUDA_WMMA(Args)                                                              \
  BENCHMARK_TEMPLATE(CUDA_WMMA_SEGMENTED_REDUCTION_16N_BLOCK, 1)                         \
      ->Args()                                                                           \
      ->UseManualTime();                                                                 \
  BENCHMARK_TEMPLATE(CUDA_WMMA_SEGMENTED_REDUCTION_16N_BLOCK, 2)                         \
      ->Args()                                                                           \
      ->UseManualTime();                                                                 \
  BENCHMARK_TEMPLATE(CUDA_WMMA_SEGMENTED_REDUCTION_16N_BLOCK, 4)                         \
      ->Args()                                                                           \
      ->UseManualTime();                                                                 \
  BENCHMARK_TEMPLATE(CUDA_WMMA_SEGMENTED_REDUCTION_16N_BLOCK, 8)                         \
      ->Args()                                                                           \
      ->UseManualTime();                                                                 \
  BENCHMARK_TEMPLATE(CUDA_WMMA_SEGMENTED_REDUCTION_16N_BLOCK, 16)                        \
      ->Args()                                                                           \
      ->UseManualTime();
