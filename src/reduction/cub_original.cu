...
static void ORIGINAL_CUB_FULL_REDUCTION(benchmark::State &state) {
  ...
  if (errors > 0) {
    printf("ORIGINAL_CUB_FULL_REDUCTION does not agree with SEQUENTIAL! %d errors!\n",
           errors);
  } else {
...
BENCHMARK(ORIGINAL_CUB_FULL_REDUCTION)->ARGS()->UseManualTime();
